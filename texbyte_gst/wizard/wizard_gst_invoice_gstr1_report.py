    ...
    def generate_b2b_report(self, wb1):
        ...
        for invoice in self.sorted_invoices.filtered(lambda p: p.partner_id.vat and not p.refund_invoice_id):  #GST registered customers
            ...
            for invoice_line in invoice.invoice_line_ids:
                ...
                line_taxes = invoice_line.invoice_line_tax_ids.compute_all(line_price, invoice.currency_id, invoice_line.quantity, invoice_line.product_id, invoice.partner_id)
    ...
    def generate_exempted_report(self, wb1):
        ...
        for invoice in self.sorted_invoices:
            for invoice_line in invoice.invoice_line_ids.filtered(lambda l: l.product_id.default_code not in ('ADVANCE','CHARGES','DISCOUNT')):
                line_price = invoice_line.price_unit * (1 - (invoice_line.discount or 0.0) / 100.0)
                taxes = invoice_line.invoice_line_tax_ids.compute_all(line_price, invoice.currency_id, invoice_line.quantity, invoice_line.product_id, invoice.partner_id)
