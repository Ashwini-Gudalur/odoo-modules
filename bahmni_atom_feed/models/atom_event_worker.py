...
#         patient_ref = vals.get("ref")
        try:
            if category == "create.customer":
                ...
                patient_ref = vals.get("customer_id")
                existing_customer = self.env['res.partner'].search([('ref', '=', patient_ref)])
                if existing_customer:
                    _logger.info("calling _create_or_update_person_attributes from process_event")
                    _logger.info (" and vals are %s", vals)
                    self._create_or_update_person_attributes(existing_customer.id,vals)
                _logger.info("calling create orders from update.attributes")
            return {'success': True}    
    ...
    def _create_or_update_customer(self, vals):
        ...
        if existing_customer:
            ...
            _logger.info("calling _create_or_update_person_attributes from _create_or_update_customer")
            _logger.info (" and vals are %s", vals)
            self._create_or_update_person_attributes(existing_customer.id,vals)
        else:
    ...
    def _get_customer_vals(self, vals):
        ...
        # _logger.info(".................................address_details ========= %s", json.loads(address_data))
        # _logger.info(".................................address_details ========= %s", type(json.loads(address_data)))
    ...
    def _create_or_update_person_attributes(self, cust_id, vals):#TODO whole method
        ...
        visitattributes = json.loads(vals.get("visitAttributes", "{}"))
        _logger.info(".................................visitattributes are ========= %s", visitattributes)
        attributes.update(visitattributes)
        if visitattributes.get('claimtype',False):
            # claimtype_dict={
            # "Sickle Cell" :'1',
            # "Bed Grant":'2',
            # "CMCHIS":'3'
            # }
            # attributes.update({'claimtype':claimtype_dict[str(visitattributes.get('claimtype'))]})
            claimtype = self.env['claim.type'].search([('name', '=', visitattributes.get('claimtype'))])
            if not claimtype:
                new_claimtype = self.env['claim.type'].create({'name': visitattributes.get('claimtype')})
                attributes.update({'claimtype':new_claimtype.id})
            else:
                attributes.update({'claimtype':claimtype.id})
        ...
        #existing_attribute = self.pool.get('res.partner.attributes').search([('partner_id' , '=', cust_id)])
        existing_attribute = self.env['res.partner.attributes'].search([('partner_id', '=', cust_id)])  
        ...
        if visitattributes.get('claimtype',False):
            ...
            # "CMCHIS":'3'
            # }
            # column_dict.update({'claimtype':claimtype_dict[str(visitattributes.get('claimtype'))]})
            claimtype = self.env['claim.type'].search([('name', '=', visitattributes.get('claimtype'))])
            if not claimtype:
                new_claimtype = self.env['claim.type'].create({'name': visitattributes.get('claimtype')})
                column_dict.update({'claimtype':new_claimtype.id})
            else:
                column_dict.update({'claimtype':claimtype.id})
            # if column_dict.get("x_claimtype",False):
                # column_dict.pop("x_claimtype")
        

        if existing_attribute:
            # existing_attribute.unlink()
            #instead of unlinking how to update existing_attribute?
            _logger.info("............................existing_attribute..........exists %s",existing_attribute)
            _logger.info("............................column_dict..........update %s",column_dict)
            existing_attribute.update(column_dict)
        else:
            _logger.info("............................existing_attribute..........does not exist")
            _logger.info("............................column_dict..........update %s",column_dict)
