...
class OrderSaveService(models.Model):
            ...
            for orderType, ordersGroup in groupby(all_orders, lambda order: order.get('type')):
                ...
                if(len(unprocessed_non_dispensed_order) > 0):
                    ...
                    if(not sale_order_ids):
                        ...
                        partner_attri_cnt=self.env['res.partner.attributes'].search([('partner_id','=',cus_id.id)])
                        _logger.info(".................................partner_attri_cnt========= %s", partner_attri_cnt)
                        if partner_attri_cnt:
                            sale_order_vals.update({'claimtype' : partner_attri_cnt.claimtype.id})
    ...
    def _create_sale_order_line_function(self, sale_order, order):
        ...
        if(stored_prod_ids):
            ...
            if prod_lot:
                location_stock_quant_records = self.env['location.stock.quant'].search([('lot_id','=',prod_lot.id)
                        ,('product_id','=',prod_id)
                        ,('location_id','=',sale_obj.shop_id.location_id.id)],order='life_date asc', limit=1)
                if location_stock_quant_records:
                    sale_order_line['location_lot_line_id'] = location_stock_quant_records.id
            sale_line = sale_order_line_obj.create(sale_order_line)
            sale_line.onchange_location_lot_line_id()
            sale_line.onchange_lot_id()
